...
void InstancesRender::DrawFrameSimple(bool draw_gui)
{
  ...
  VK_CHECK_RESULT(vkQueueSubmit(m_context->getQueue(),
    1,
    &submitInfo,
    m_frameFences[m_presentationResources.currentFrame]));
...
void InstancesRender::DrawFrame(float a_time, DrawMode a_mode)
{
  VkDrawIndexedIndirectCommand* data = reinterpret_cast<VkDrawIndexedIndirectCommand*>(m_drawCommandBuffer.data());
  memset(data, 0, sizeof(data));
  if (m_pScnMgr->InstancesNum() >= 1)
  {
    auto instance = m_pScnMgr->GetInstanceInfo(0);

    auto meshInfo = m_pScnMgr->GetMeshInfo(instance.mesh_id);

    data->indexCount = meshInfo.m_indNum;
    data->instanceCount = 0;
    data->firstIndex = meshInfo.m_indexOffset;
    data->vertexOffset = meshInfo.m_vertexOffset;
    data->firstInstance = 0;

    m_frustumCullingParams.mViewProjection = m_worldViewProj;

    Box4f box = m_pScnMgr->GetMeshBbox(instance.mesh_id);
    m_frustumCullingParams.boundingBoxBegin = box.boxMin;
    m_frustumCullingParams.boundingBoxEnd = box.boxMax;
  }
